...
public class Percolation {
    ...
    // Declare another weightedQuickUnionUF object to avoid backwash.
    ...
    public Percolation(int n) {
        ...
        // Object to store connected components.
    ...
    public void open(int row, int col) {
        ...
        if (!grid[row-1][col-1]) {
            ...
            if (row == 1) {
                ...
                backwash.union(siteNumber(row, col), top);
            }
            ...
            if (col < n && isOpen(row, col+1)) {
                ...
                backwash.union(siteNumber(row, col), siteNumber(row, col+1));
            }

            if (row < n && isOpen(row+1, col)) {
                weightedQuickUnionUF.union(siteNumber(row, col), siteNumber(row+1, col));
                backwash.union(siteNumber(row, col), siteNumber(row+1, col));
            }
    ...
    public boolean isFull(int row, int col) {
        return backwash.connected(siteNumber(row, col), top);
    }
